...
export default function ForgotPassword() {
  ...
  const dm      = darkMode;
  const pageBg  = dm ? "bg-gray-950" : "bg-white";
  ...
  const inputCls = dm
    ? "bg-gray-700 border-gray-600 text-gray-100 placeholder:text-gray-500 rounded-xl w-full px-3 py-2.5 text-sm border outline-none focus:ring-2 focus:ring-[#4A90E2] focus:border-[#4A90E2] transition-colors"
    : "bg-slate-50 border-slate-200 text-gray-900 rounded-xl w-full px-3 py-2.5 text-sm border outline-none focus:ring-2 focus:ring-[#4A90E2] focus:border-[#4A90E2] transition-colors";
  const btnCls = "w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-[#4A90E2] hover:opacity-90 text-white text-sm font-semibold transition-all shadow-md shadow-[#4A90E2]/30 disabled:opacity-60 disabled:cursor-not-allowed";
    ...
    <div dir={language === "ar" ? "rtl" : "ltr"}
      ...
      <div className="w-full max-w-sm space-y-5">
        ...
        <div className="flex flex-col items-center mb-2">
          <div className="w-16 h-16 rounded-2xl bg-[#4A90E2] flex items-center justify-center shadow-lg shadow-[#4A90E2]/30 mb-3">
              ...
              <div key={i} className="flex items-center gap-2">
                <div className="flex flex-col items-center">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                    i < stepIndex ? "bg-[#4A90E2] text-white" :
                    i === stepIndex ? "bg-[#4A90E2] text-white ring-2 ring-[#4A90E2]/40" :
                    dm ? "bg-gray-700 text-gray-400" : "bg-slate-200 text-slate-400"
                  }`}>
                    {i < stepIndex ? "✓" : i + 1}
                  </div>
                  <span className={`text-xs mt-1 ${i === stepIndex ? "text-[#4A90E2] font-medium" : sub}`}>{label}</span>
                </div>
                {i < 2 && (
                  <div className={`w-8 h-0.5 mb-4 transition-all ${i < stepIndex ? "bg-[#4A90E2]" : dm ? "bg-gray-700" : "bg-slate-200"}`} />
                )}
              </div>
            ))}
          </div>
          ...
          {step === "otp" && (
            <>
              <div>
                ...
                {/* OTP boxes — 4 + 4 */}
                <div className="flex items-center justify-center gap-3" dir="ltr" onPaste={handleOtpPaste}>
                  {[0, 1].map(group => (
                    <div key={group} className="flex gap-1.5">
                      {otp.slice(group * 4, group * 4 + 4).map((digit, j) => {
                        const i = group * 4 + j;
                        return (
                          <input
                            key={i}
                            ...
                            className={`w-10 h-12 text-center text-lg font-bold rounded-xl border-2 outline-none transition-all ${
                              digit
                                ? dm ? "border-[#4A90E2] bg-[#4A90E2]/20 text-[#4A90E2]" : "border-[#4A90E2] bg-[#EAF3FF] text-[#4A90E2]"
                                : dm ? "border-gray-600 bg-gray-700 text-gray-100" : "border-slate-200 bg-slate-50 text-gray-900"
                            } focus:border-[#4A90E2] focus:ring-2 focus:ring-[#4A90E2]/30`}
                          />
                        );
                      })}
                    </div>
                ...
                <p className={`text-xs text-center mt-3 ${sub}`}>
                  ...
                  <button onClick={() => { setOtp(["","","","","","","",""]); setStep("email"); setError(""); }}
                    className="text-[#4A90E2] hover:opacity-80 transition-colors">
